...
#calculate the amount per gamble gamble
  ...
  #function to print the per day gambling result after 20 days
    printingAfter20Days(){
        ...
        if [ $no_of_days -ge 20 ]
	then
            echo "Total amount won after " $no_of_days " days:" totalCalculation($nofdayswon $currentBalance)
            echo "Total amount lost after " $no_of_days  " days: " totalCalculation($nofdayslost $currentBalance)
        fi
     }
